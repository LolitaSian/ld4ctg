...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        if self.diffusion_model.class_conditional:
            if self.diffusion_model.class_unconditional_prob > 0:
                self.class_unconditional_bernoulli = torch.distributions.Bernoulli(
                    probs=self.diffusion_model.class_unconditional_prob)
        ...
        '''
        register_buffer 函数：这是一个用于将变量注册为模型缓冲区（不需要梯度更新）的辅助函数。
        '''
    ...
    def diffusion_model_predictions(self, x, mask, t, x_self_cond=None, class_id=None):
        '''
        根据目标类型（'pred_noise' 或 'pred_x0'）来预测噪声或图像的起始状态。
        '''
    ...
    def p_losses(self, x_start, mask, t, class_id, noise=None):
        ...
        loss = rearrange([reduce(loss[i][:torch.sum(mask[i])], 'l d -> 1', 'mean') for i in range(x_start.shape[0])],
                         'b 1 -> b 1')
...
class Trainer(object):
    def __init__(
        ...
        training_lengths = [min(sum(self.dataloader.dataset[idx]['attention_mask']), self.max_seq_len) for idx in
                            range(self.dataloader.dataset.num_rows)]
        length_counts = Counter(training_lengths)
        probs = torch.tensor(
            [length_counts[idx] / self.dataloader.dataset.num_rows for idx in range(self.max_seq_len + 1)])
        ...
        if self.diffusion.diffusion_model.class_conditional:
            ...
            probs = torch.tensor([label_counts[idx] / self.dataloader.dataset.num_rows for idx in
                                  range(self.diffusion.diffusion_model.num_classes)])
            self.class_categorical = torch.distributions.Categorical(probs=probs)

        # optimizer

        self.opt = optimizer.get_adamw_optimizer(diffusion.parameters(), lr=train_lr, betas=adam_betas,
    ...
    def log_reference_metrics(self, test=False):
        ...
        if test:
            ...
            self.reference_dict[f"reference/test_memorization"] = evaluation.compute_memorization(test_subset,
                                                                                                  self.dataset['train'][
                                                                                                      'text'])
        ...
        for mauve_model_id in ["gpt2-large", "all-mpnet-base-v2"]:
            self.reference_dict[f'reference/{mauve_model_id}_train_val_mauve'], _ = evaluation.compute_mauve(
                train_subset, val_subset, mauve_model_id)
            self.reference_dict[f'reference/{mauve_model_id}_train_train_mauve'], _ = evaluation.compute_mauve(
                train_subset, train_subset2, mauve_model_id)
        ...
        self.reference_dict[f"reference/val_memorization"] = evaluation.compute_memorization(val_subset,
                                                                                             self.dataset['train'][
                                                                                                 'text'])
    ...
    def gen_synthetic_dataset(self, num_samples, seed=42):
        ...
        for class_id in range(num_classes):
            ...
            while len(text) < num_samples_per_class:
                ...
                model_outputs = list(map(lambda n: tuple(x.to('cpu') for x in self.ema.ema_model.sample(batch_size=n,
                                                                                                        length=self.length_categorical.sample(
                                                                                                            (n,)),
    ...
    def sample(self, num_samples=None, class_id=None, seed=42, test=False):
        ...
        def get_class_id(n):
            ...
            if self.diffusion.diffusion_model.class_conditional:
                if self.diffusion.diffusion_model.class_unconditional_prob > 0:
                    return torch.tensor([self.diffusion.diffusion_model.num_classes] * n, dtype=torch.long,
                                        device=device)
        ...
        while min([len(all_texts_lists[ele]) for ele in all_texts_lists]) < num_samples:
            batches = num_to_groups(num_samples - min([len(all_texts_lists[ele]) for ele in all_texts_lists]),
        ...
        for strategy, all_texts_list in text_generations.items():
            ...
            file_utils.save_text_samples(all_texts_list, os.path.join(self.results_folder,
                                                                      f'{"eval-" if self.args.eval else ""}{f"eval{seed}-" if self.args.eval_test else ""}{class_id_prefix}{strategy}-sample-{milestone}.txt'))
            metrics[f"model/{strategy}/{class_id_prefix}perplexity"] = evaluation.compute_perplexity(all_texts_list)
            metrics[f"model/{strategy}/{class_id_prefix}unique_wordcount"] = evaluation.compute_wordcount(
                all_texts_list)
            ngram_metrics = evaluation.compute_diversity(all_texts_list)
            for k, v in ngram_metrics.items():
                metrics[f"model/{strategy}/{class_id_prefix}{k}"] = v
            metrics[f"model/{strategy}/{class_id_prefix}memorization"] = evaluation.compute_memorization(all_texts_list,
                                                                                                         self.dataset[
                                                                                                             'train'][
                                                                                                             'text'])
            table = wandb.Table(
            ...
            while self.step < self.train_num_steps:
                ...
                for grad_accum_step in range(self.gradient_accumulate_every):
                        ...
                        if self.args.normalize_latent:
                            if self.step == 0 and grad_accum_step == 0:
                                latent_vecs = torch.cat(
                                    [latent[i][:torch.sum(data['attention_mask'][i])] for i in range(latent.shape[0])],
                                    dim=0)
                                ...
                                self.diffusion.latent_scale = torch.std(latent_vecs - self.diffusion.latent_mean,
                                                                        unbiased=False)
                    ...
                    with self.accelerator.autocast():
                        loss = self.diffusion(latent, mask, class_id=(
                            data['label'] if self.diffusion.diffusion_model.class_conditional else None))
                        ...
                        with torch.no_grad():
                            ...
                            for grad_accum_step in range(self.gradient_accumulate_every):
                                ...
                                with self.accelerator.autocast():
                                    ...
                                    loss = self.ema.ema_model(latent, mask, class_id=(
                                        data['label'] if self.diffusion.diffusion_model.class_conditional else None))
                                    loss = loss / self.gradient_accumulate_every
                                    total_val_ema_loss += loss.item()

                            logs = {"loss": total_loss, "val_loss": total_val_loss, "val_ema_loss": total_val_ema_loss,
                                    "grad_norm": grad_norm, "lr": self.lr_scheduler.get_last_lr()[0], "step": self.step,
